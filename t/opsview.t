...
for my $var (qw/ url username password /) {
    ...
    if ( !$ENV{$envvar} ) {
        note "Using default '$envvar' value of '$opsview{$var}' for testing.";
    }
    else {
...
# object was create, we tried to access it, but the URL was not to an Opsview server
if($trap->die && ref($trap->die) eq 'Opsview::RestAPI::Exception') {
    if($trap->die->message =~ m/was not found on this server/ || $trap->die->http_code != 200) {
        my $message = "HTTP STATUS CODE: ".$trap->die->http_code. " MESSAGE: ".$trap->die->message;
        $message =~ s/\n/ /g;
        BAIL_OUT("The configured URL '$opsview{url}' does NOT appear to be an opsview server: ". $message);
    }
}
...
like($output->{api_min_version}, qr/^\d\.\d$/, "api_version 'api_min_version' returned okay");
like($output->{api_version}, qr/^\d\.\d+$/, "api_version 'api_version' returned okay");
like($output->{easyxdm_version}, qr/^\d\.\d\.\d+$/, "api_version 'easyxdm_version' returned okay");

note("Got 'api_version' from '$opsview{url}' of ".$output->{api_version});
...
is( $trap->die, "Not logged in", "Exception stringified to 'Not logged in' correctly");
...
like( $output->{opsview_version}, qr/^\d\.\d+\.\d$/, "opsview_info 'opsview_version' returned okay");
like( $output->{opsview_build}, qr/^\d\.\d+\.\d\.\d+$/, "opsview_info 'opsview_build' returned okay");
like( $output->{opsview_edition}, qr/^\w+$/, "opsview_info 'opsview_edition' returned okay");
like( $output->{uuid}, qr/^\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$/, "opsview_info 'uuid' returned okay");
